...
Export a WhatsApp chat **with media**, then upload the ZIP. The library supports searching, reopening, and copying each conversation's permanent random link. Every conversation link requires the workspace password. Anonymous readers see the login form at the same address and return to that conversation after signing in. Attachment endpoints also require a valid session; direct image, video, PDF, download, and byte-range requests return 401 when signed out. The single shared password grants access to the administrator workspace too; there is no separate viewer role. This version does not implement per-person permissions or expiring links. To revoke one archive, stop the server and remove its matching `data/<id>.json` and `data/<id>.zip` pair after backing up if needed.
